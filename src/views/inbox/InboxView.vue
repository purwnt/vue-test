...
export default {
  ...
  methods: {
    markAsRead() {
      this.emailStoreStore.markAsRead();
    },
    markAsarchive() {
      this.emailStoreStore.markAsArchive();
    },
  ...
  <div class="inbox">
    <Header
      ...
      :subheading="`Email Selected (${
        emailStoreStore.data.filter((el) => el.isChecked && !el.isArchive)
          .length
      })`"
    ></Header>
    <div class="inbox__tools">
      ...
      <button @click="markAsarchive()">Archive (a)</button>
    </div>
    <div class="inbox__content">
      ...
      >
        ...
        />
        <p
          class="inbox__content__item__subject"
          @click="openReadEmailModal(item.id)"
        >
          {{ item.subject }}
        </p>
      </div>
